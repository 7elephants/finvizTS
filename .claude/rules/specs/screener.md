...
- `f` (optional): Filter string for the `f` query parameter. Narrows the screener results by one or more criteria.

  **Format rules:**
  - Multiple options for the **same filter type** are OR'd together using `|`, with the shared prefix written only once:
    `exch_amex|nasd` (not `exch_amex|exch_nasd`)
  - Multiple **different filter types** are AND'd together, separated by `,`:
    `exch_nasd,geo_usa`
  - Combined: `exch_amex|nasd,geo_usa`

  **Building filter strings:**
  Use the `buildFilters()` utility and typed filter constants exported from the package to compose this string without manual string manipulation:

  ```typescript
  import { buildFilters, ScreenerExchangeFilter, ScreenerCountryFilter } from 'finviz-ts';

  // Single filter value (AND'd):
  buildFilters([ScreenerExchangeFilter.NASDAQ, ScreenerCountryFilter.USA])
  // → 'exch_nasd,geo_usa'

  // Multiple values for one filter type (OR'd), AND'd with another:
  buildFilters([[ScreenerExchangeFilter.AMEX, ScreenerExchangeFilter.NASDAQ], ScreenerCountryFilter.USA])
  // → 'exch_amex|nasd,geo_usa'
  ```

  Each element in the `buildFilters` array is one filter type:
  - A **string** → single value, passed through as-is
  - A **string[]** → multiple values for the same filter type, OR'd together with prefix-stripping

- `s` (optional): Signal filter. Shortcut for common technical setups (e.g. `ta_topgainers`, `ta_toplosers`, `ta_newhigh`, `ta_newlow`). Applied in addition to any `f` filters.

- `o` (optional): The column the results should be sorted by. Prefix with `-` for descending order (e.g. `o=-price`).
